...
El backlog de requisitos reúne **50 historias en total: 42 User Stories funcionales y 8 Technical Stories**. Las User Stories mantienen el foco en el problema y el valor para el usuario sin depender del framework de implementación. Las Technical Stories se reservan para capacidades del RESTful API, de acuerdo con el Final Project Statement del curso. Las historias de GPS continuo, ETA dinámico y geofencing se mantienen como **roadmap posterior al MVP**, de modo que el núcleo inicial de Rumbo siga centrado en estados, hitos, confirmaciones, retrasos, incidencias y notificaciones.\n\nPara Aplicaciones Web, la solución se implementará con **Vue 3 + JavaScript + PrimeVue** en la Frontend Web Application, siguiendo **Material Design** como lenguaje visual. El backend se implementará como **RESTful API con ASP.NET Core, C# y Entity Framework Core**, documentado mediante **OpenAPI/Swagger**. Estas decisiones técnicas no cambian el significado de las User Stories; únicamente condicionan las Technical Stories y la implementación posterior.
...
| TS04 | Internacionalización del RESTful API | Como Developer, deseo internacionalizar los mensajes y respuestas del RESTful API para soportar inglés y español latinoamericano de forma consistente. | **Escenario 1:** Given una solicitud sin preferencia de idioma, When el API genera un mensaje de validación o error, Then utiliza inglés como idioma predeterminado. // **Escenario 2:** Given una solicitud con preferencia es_419 soportada, When el API genera un mensaje localizado, Then devuelve el contenido disponible en español latinoamericano sin cambiar la estructura del response. | EP07 |
| TS05 | Paginación y filtrado en endpoints de estudiantes | Como Developer, deseo implementar paginación y filtros en los endpoints ASP.NET Core de estudiantes para mantener respuestas manejables y eficientes. | **Escenario 1:** Given una consulta paginada válida, When el endpoint procesa la solicitud, Then retorna los registros y metadatos de paginación correspondientes. // **Escenario 2:** Given un filtro válido, When se realiza la consulta, Then el servicio retorna únicamente los registros que cumplen el criterio. | EP07 |
...
| 45 | TS04 | Internacionalización del RESTful API | Entregar mensajes y errores localizados en en_US y es_419, con inglés como idioma predeterminado. | 3 |
