...
A continuación se presenta el conjunto de Epics y User Stories de Rumbo. Los criterios de aceptación siguen la estructura Gherkin (Given-When-Then), se redactan en tiempo presente y tercera persona, y no hacen referencia a detalles de interfaz de usuario. Las Technical Stories corresponden a las capacidades del RESTful API y utilizan el rol Developer.
...
| EP02 | Suscripción del conductor | Activación, vigencia y renovación del plan que habilita al conductor el uso de las funcionalidades de Rumbo. | — | — |
...
| EP07 | Servicios backend e integraciones | Capacidades del RESTful API, integraciones con servicios externos de mapas, notificaciones y verificación de credenciales, y atributos técnicos de seguridad, documentación y trazabilidad. | — | — |
...
| US09 | Activar la suscripción del conductor | Como conductor, deseo activar una suscripción para habilitar las funcionalidades incluidas en mi plan. | **Escenario 1: Activación exitosa**<br>Given un conductor con cuenta activa y un plan seleccionado<br>When el pago es aprobado<br>Then el sistema activa la suscripción por el periodo contratado y habilita las funcionalidades del plan<br><br>**Escenario 2: Pago rechazado**<br>Given un intento de pago rechazado por la pasarela<br>When el sistema recibe el resultado<br>Then informa el rechazo y conserva el estado previo de la suscripción<br><br>**Escenario 3: Suscripción vencida**<br>Given una suscripción cuyo periodo ha concluido<br>When el conductor intenta usar una funcionalidad del plan<br>Then el sistema restringe el acceso e informa que debe renovar su suscripción | EP02 |
...
| US11 | Definir el horario de la ruta | Como conductor, deseo establecer los días y horarios de una ruta para que sus viajes se programen de forma recurrente. | **Escenario 1: Horario válido**<br>Given una ruta en estado configurable<br>When el conductor define los días de servicio y las horas de salida<br>Then el sistema guarda el horario y lo asocia a la ruta<br><br>**Escenario 2: Horarios superpuestos**<br>Given que el conductor ya tiene otra ruta activa en el mismo día y franja horaria<br>When intenta guardar el nuevo horario<br>Then el sistema advierte la superposición antes de confirmarla | EP03 |
| US12 | Asignar estudiantes a una ruta | Como conductor, deseo asignar a los estudiantes autorizados a una ruta y a su parada para incluirlos en los recorridos. | **Escenario 1: Asignación autorizada**<br>Given un estudiante cuyo tutor autorizó la vinculación y una ruta con capacidad disponible<br>When el conductor lo asigna a una parada de la ruta<br>Then el sistema registra la asignación y el estudiante queda incluido en los viajes de esa ruta<br><br>**Escenario 2: Capacidad del vehículo alcanzada**<br>Given una ruta cuyos estudiantes asignados igualan la capacidad registrada del vehículo<br>When el conductor intenta asignar un estudiante adicional<br>Then el sistema impide la asignación e informa que no existe capacidad disponible<br><br>**Escenario 3: Sin autorización del tutor**<br>Given un estudiante cuya vinculación no ha sido autorizada por su tutor<br>When el conductor intenta asignarlo<br>Then el sistema rechaza la operación y mantiene la solicitud pendiente de autorización<br><br>**Escenario 4: Retiro de un estudiante**<br>Given un estudiante asignado a una ruta<br>When el conductor retira la asignación<br>Then el estudiante deja de figurar en los viajes que se generen posteriormente | EP03 |
...
| US14 | Modificar una ruta publicada | Como conductor, deseo actualizar una ruta publicada para mantenerla alineada con mi operación real. | **Escenario 1: Modificación aplicada**<br>Given una ruta publicada<br>When el conductor modifica sus paradas, su orden o su horario<br>Then el sistema guarda el cambio y lo aplica a los viajes que se generen en adelante<br><br>**Escenario 2: Viajes en curso**<br>Given un viaje activo generado a partir de la ruta<br>When el conductor modifica la ruta<br>Then el viaje en curso conserva la configuración con la que fue iniciado<br><br>**Escenario 3: Comunicación del cambio**<br>Given una modificación que altera una parada o un horario<br>When el cambio es guardado<br>Then el sistema informa a los tutores de los estudiantes afectados | EP03 |
...
| US16 | Reportar la ausencia del estudiante | Como padre o tutor, deseo informar que mi hijo no usará la movilidad para evitar una parada innecesaria. | **Escenario 1: Ausencia antes del inicio**<br>Given un viaje programado que aún no ha iniciado<br>When el tutor reporta la ausencia del estudiante para esa fecha<br>Then el sistema actualiza la lista del viaje y excluye su recojo<br><br>**Escenario 2: Ausencia con el viaje en curso**<br>Given un viaje ya iniciado cuya parada del estudiante no ha sido alcanzada<br>When el tutor reporta la ausencia<br>Then el sistema actualiza la lista y comunica el cambio al conductor<br><br>**Escenario 3: Ausencia programada con anticipación**<br>Given fechas futuras con viajes programados<br>When el tutor registra la ausencia para un rango de fechas<br>Then el sistema excluye al estudiante de los viajes comprendidos en ese rango<br><br>**Escenario 4: Cancelación de la ausencia**<br>Given una ausencia registrada para un viaje que aún no ha iniciado<br>When el tutor la cancela<br>Then el estudiante vuelve a figurar en la lista del viaje | EP03 |
...
| US18 | Iniciar el viaje | Como conductor, deseo iniciar el recorrido para que los tutores sepan que la ruta está en ejecución. | **Escenario 1: Inicio del recorrido**<br>Given un viaje programado para la fecha y turno actuales<br>When el conductor confirma el inicio<br>Then el sistema registra la hora de inicio, cambia el estado del viaje a en ejecución e informa a los tutores<br><br>**Escenario 2: Viaje ya iniciado**<br>Given un viaje en ejecución<br>When se intenta iniciarlo nuevamente<br>Then el sistema mantiene el estado vigente y no registra un segundo inicio<br><br>**Escenario 3: Inicio fuera del horario previsto**<br>Given un viaje que se inicia con una diferencia significativa respecto de su horario programado<br>When el conductor confirma el inicio<br>Then el sistema registra la diferencia como parte del estado del viaje | EP04 |
| US19 | Registrar los hitos de una parada | Como conductor, deseo confirmar los recojos de cada parada en pocos segundos para dejar constancia sin afectar mi recorrido. | **Escenario 1: Recojo confirmado**<br>Given un viaje en ejecución, el vehículo detenido en una parada y un estudiante previsto en ella<br>When el conductor confirma su recojo<br>Then el sistema registra el hito con fecha y hora y lo pone a disposición de los tutores autorizados<br><br>**Escenario 2: Recojo no realizado**<br>Given un estudiante previsto que no aborda la movilidad<br>When el conductor lo registra como recojo no realizado<br>Then el sistema conserva el hito y no considera al estudiante como recogido<br><br>**Escenario 3: Cierre de la parada**<br>Given una parada cuyos estudiantes previstos cuentan con un resultado registrado<br>When el conductor confirma el cierre de la parada<br>Then el sistema la marca como completada y habilita la siguiente del recorrido<br><br>**Escenario 4: Corrección de un hito**<br>Given un hito registrado por error dentro del viaje en curso<br>When el conductor lo corrige<br>Then el sistema aplica la corrección conservando el registro del hito anterior<br><br>**Escenario 5: Vehículo en movimiento**<br>Given un vehículo que no se encuentra detenido<br>When se intenta confirmar un hito<br>Then el sistema no habilita la confirmación | EP04 |
...
| US29 | Configurar las preferencias de notificación | Como padre o tutor, deseo elegir qué avisos recibir para no ser saturado con información que no necesito. | **Escenario 1: Desactivación de un aviso**<br>Given un tipo de aviso no crítico habilitado<br>When el tutor lo desactiva<br>Then el sistema deja de enviarlo y lo mantiene disponible para consulta en la plataforma<br><br>**Escenario 2: Avisos críticos**<br>Given una incidencia o una entrega no concretada<br>When se genera el aviso correspondiente<br>Then el sistema lo envía aunque otros avisos se encuentren desactivados<br><br>**Escenario 3: Preferencias por estudiante**<br>Given un tutor con más de un estudiante autorizado<br>When configura sus preferencias<br>Then el sistema aplica la configuración al estudiante seleccionado | EP05 |
...
| TS08 | Trazabilidad de los eventos del dominio | Como Developer, deseo conservar el registro de los eventos del viaje y sus cambios para garantizar la trazabilidad de la operación. | **Escenario 1: Persistencia del evento**<br>Given una operación confirmada sobre un viaje<br>When la transacción se completa<br>Then el servicio persiste el estado y el evento asociado de forma conjunta<br><br>**Escenario 2: Transacción fallida**<br>Given una operación que falla durante su ejecución<br>When la transacción se revierte<br>Then no se conserva información parcial del estado ni del evento<br><br>**Escenario 3: Consulta de auditoría**<br>Given una solicitud autorizada sobre el historial de un viaje<br>When se invoca el recurso correspondiente<br>Then el servicio devuelve la secuencia de eventos con su actor, fecha y hora | EP07 |

## 3.2. Impact Mapping

El Impact Mapping conecta los objetivos de negocio de Rumbo con los actores que pueden contribuir a alcanzarlos, los cambios de comportamiento esperados en ellos y los entregables que Rumbo debe construir para provocar dichos cambios. Los Business Goals se formulan bajo criterios SMART y se derivan de los Business Outcome Assumptions definidos en el Lean UX Process.

| Business Goal (¿Por qué?) | Actor (¿Quién?) | Impacto esperado (¿Cómo?) | Deliverables (¿Qué?) | User Stories |
|---|---|---|---|---|
| Reducir en 60 % los mensajes y llamadas al conductor para consultar el estado de la ruta, durante los 3 primeros meses del piloto. | Padre / Tutor | Consulta el estado del traslado en Rumbo en lugar de escribir al conductor. | Vista de estado actual del viaje y línea de tiempo del trayecto | Como padre o tutor, deseo conocer en pocos segundos la etapa del traslado para evitar preguntarle al conductor. (US26)<br>Como padre o tutor, deseo revisar los hitos ocurridos durante el recorrido para entender qué pasó sin revisar conversaciones. (US27) |
| Lograr que al menos el 70 % de los padres y tutores activos consulte Rumbo en 3 o más días de clases por semana. | Padre / Tutor | Recibe avisos oportunos y vuelve a la plataforma sin necesidad de revisarla constantemente. | Centro de notificaciones con preferencias configurables | Como padre o tutor, deseo recibir avisos solo cuando ocurre un evento relevante para mantenerme informado sin revisar la plataforma constantemente. (US28)<br>Como padre o tutor, deseo elegir qué avisos recibir para no ser saturado con información que no necesito. (US29) |
| Lograr que al menos el 80 % de los recojos y entregas de cada ruta quede confirmado dentro de Rumbo. | Conductor | Confirma los hitos del recorrido con una interacción breve, con el vehículo detenido. | Confirmación de hitos de parada, llegada al colegio, entrega y cierre del viaje | Como conductor, deseo iniciar el recorrido para que los tutores sepan que la ruta está en ejecución. (US18)<br>Como conductor, deseo confirmar los recojos de cada parada en pocos segundos para dejar constancia sin afectar mi recorrido. (US19)<br>Como conductor, deseo confirmar la llegada al centro educativo y dar inicio al retorno para diferenciar ambas etapas del servicio. (US20)<br>Como conductor, deseo confirmar la entrega de cada estudiante para cerrar su traslado y avisar a su tutor. (US21)<br>Como conductor, deseo cerrar el viaje para consolidar su resultado y dejarlo disponible como historial. (US22) |
| Lograr que al menos el 90 % de los retrasos e incidencias se comunique mediante Rumbo y no mediante mensajes individuales. | Conductor | Informa un imprevisto una sola vez y la plataforma lo comunica a las familias afectadas. | Registro de retrasos e incidencias con categorías y su resolución | Como conductor, deseo registrar un retraso y su causa para informar con un solo registro a todas las familias afectadas. (US23)<br>Como conductor, deseo registrar una incidencia para comunicar un imprevisto con contexto suficiente y sin repetir el mensaje a cada familia. (US24)<br>Como conductor, deseo marcar una incidencia como resuelta para informar que la situación fue normalizada. (US25)<br>Como padre o tutor, deseo confirmar que tomé conocimiento de una incidencia para que el conductor sepa que fui informado. (US30) |
| Lograr que al menos el 60 % de los conductores del piloto continúe usando Rumbo después del primer mes. | Conductor | Organiza su ruta, sus paradas y sus estudiantes dentro de la plataforma en lugar de hacerlo de forma informal. | Configuración de rutas, paradas, horarios, asignación de estudiantes y programación de viajes | Como conductor, deseo crear una ruta con sus paradas en el orden en que las recorro para organizar mi servicio. (US10)<br>Como conductor, deseo establecer los días y horarios de una ruta para que sus viajes se programen de forma recurrente. (US11)<br>Como conductor, deseo asignar a los estudiantes autorizados a una ruta y a su parada para incluirlos en los recorridos. (US12)<br>Como conductor, deseo publicar una ruta configurada para habilitar la programación de sus viajes y su visibilidad para los tutores autorizados. (US13)<br>Como conductor, deseo actualizar una ruta publicada para mantenerla alineada con mi operación real. (US14)<br>Como conductor, deseo contar con el viaje del día y la lista de estudiantes prevista para saber a quiénes debo recoger. (US15) |
| Reducir a menos del 5 % las paradas realizadas por estudiantes que no utilizarán el servicio ese día. | Padre / Tutor | Informa la ausencia de su hijo con anticipación en lugar de avisar por mensaje o no avisar. | Reporte de ausencias y cancelación de viajes | Como padre o tutor, deseo informar que mi hijo no usará la movilidad para evitar una parada innecesaria. (US16)<br>Como conductor, deseo cancelar un viaje que no se realizará para que las familias no esperen un servicio inexistente. (US17) |
| Alcanzar 40 conductores registrados con al menos una ruta publicada durante los 6 primeros meses de operación. | Visitante | Comprende la propuesta de valor de Rumbo e ingresa al registro correspondiente a su segmento. | Landing Page con propuesta de valor, beneficios por segmento y accesos diferenciados | Como visitante, deseo comprender qué es Rumbo y qué problema resuelve para decidir si me resulta relevante. (US31)<br>Como visitante, deseo conocer los beneficios correspondientes a mi perfil e ingresar a la experiencia que me corresponde. (US32)<br>Como visitante, deseo resolver mis dudas o comunicarme con el equipo para decidir si utilizo el servicio. (US35) |
| Lograr que el 100 % de los accesos a la información de un estudiante provenga de tutores con autorización vigente. | Padre / Tutor | Controla quién puede consultar la información de su hijo y autoriza su vinculación con un conductor. | Gestión de cuentas, autorización de tutores y control de acceso por rol | Como padre o tutor, deseo registrar a mi hijo y quedar vinculado como su tutor para poder consultar la información de sus traslados. (US06)<br>Como padre o tutor, deseo autorizar o retirar el acceso de otro tutor sobre mi hijo para controlar quién puede consultar su información. (US07)<br>Como padre o tutor, deseo solicitar la eliminación de los datos personales de mi hijo para ejercer el control sobre su información. (US08) |
| Conseguir que al menos el 50 % de los conductores del piloto active una suscripción al término del periodo de prueba. | Conductor | Contrata un plan para mantener habilitadas las funcionalidades de Rumbo. | Activación y vigencia de la suscripción del conductor | Como conductor, deseo activar una suscripción para habilitar las funcionalidades incluidas en mi plan. (US09) |
