...
El backlog de requisitos se mantiene en **50 historias en total: 42 User Stories funcionales y 8 Technical Stories**. Las User Stories conservan el alcance funcional trabajado en Rumbo; para Aplicaciones Web, las Technical Stories se enfocan en features del RESTful API y sus escenarios request/response. Las historias de GPS continuo, ETA dinámico y geofencing se mantienen como **roadmap posterior al MVP**, de modo que el núcleo inicial de Rumbo siga centrado en estados, hitos, confirmaciones, retrasos, incidencias y notificaciones.
...
| TS04 | Internacionalización del RESTful API | Como Developer, deseo internacionalizar los mensajes y respuestas del RESTful API para soportar `en_US` y `es_419`. | **Escenario 1:** Given una solicitud sin preferencia de idioma, When el API genera un mensaje de validación o error, Then utiliza inglés como idioma predeterminado. // **Escenario 2:** Given una solicitud con preferencia `es_419`, When el API genera un mensaje localizado disponible, Then devuelve el contenido en español latinoamericano sin alterar la estructura del response. | EP07 |
...
| 45 | TS04 | Internacionalización del RESTful API | Localizar mensajes de API para `en_US` y `es_419`, usando inglés por defecto. | 3 |
